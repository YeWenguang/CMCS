...
#     memory_limit_fraction = 36864 * 1024 * 1024 / total_memory 
#     torch.cuda.set_per_process_memory_fraction(memory_limit_fraction, 0)  
...
model_id2 = "Artigenz/Artigenz-Coder-DS-6.7B"
...
file_path = "Your_file_path/SPoC/test-testw.txt""
...
def extract_cpp_code(terminal_output, index):
    ...
    for code_block in reversed(matches_code):
        if '#include <iostream>' in code_block or '#include<iostream>' in code_block:
            ...
            if all('```' not in line for line in lines[:-1]): 
...
def run_cpp_with_input(executable_filename, input_data):
    ...
    try:
        ...
        last_colon_index = stdout.rfind(":")  
        if last_colon_index != -1:
            return stdout[last_colon_index + 1:].strip()  
        else:
            return stdout.strip()  
